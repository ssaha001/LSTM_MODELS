...
dataset['Month']=dataset['Month'].str.replace('-','').astype(dtype='float32')
...
x_train_data_tensor,x_test_data_tensor=torch.tensor(x_train_data.values).unsqueeze(dim=0),torch.tensor(x_test_data.values).unsqueeze(dim=0)
y_train_data_tensor,y_test_data_tensor=torch.tensor(y_train_data.values).unsqueeze(dim=0),torch.tensor(y_test_data.values).unsqueeze(dim=0)
...
class LSTM(nn.Module):
    def __init__(self,input_size,hidden_size):
        ...
        self.first_layer=nn.LSTMCell(1,self.hidden_size)
    ...
    def forward(self,x_data):
        ...
        for input_tensor in x_train_data_tensor.split(1,dim=1):
